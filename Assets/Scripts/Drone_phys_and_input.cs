...
public class Drone_phys_and_input : MonoBehaviour
{
    ...
    public float my_phys_turn_off_duration = 0.2f; //длительность отключени€ физики при ударе
    ...
    Rigidbody rb;
    //public float maxAngularVelocity = 3.14f/180 * 1000; //максимальна€ углова€ скорость рад/с при которой управл€етс€ коптер. ≈сли она больше, то ждем пока уменьшитс€ драгом
    ...
    {        
        Turn_off_my_phys(); //выключить мою физику на врем€
        speed = collision.GetContact(0).normal; //скорость когда включитс€ физика
        Invoke("Turn_on_my_phys", my_phys_turn_off_duration); //на случай, если коллайдер удалитс€ (ракета) и выхода из него не будет
    }
